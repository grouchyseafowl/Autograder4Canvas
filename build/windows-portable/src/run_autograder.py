...
def save_canvas_url_permanently(canvas_url):
    ...
    if system == "Windows":
        # Portable mode: write to credentials.bat if it exists next to run.bat
        ...
        if creds_file and os.path.exists(creds_file):
            try:
                ...
                with open(creds_file, 'w', encoding='utf-8') as f:
                    f.writelines(lines)
                print(f"Canvas URL saved to credentials.bat")
            except Exception as e:
                print(f"Could not save to credentials.bat: {e}")
        else:
            print()
            print("To save permanently, open credentials.bat in Notepad and")
            print(f"set CANVAS_BASE_URL={canvas_url}")
        return
...
def save_token_permanently(token):
    ...
    if system == "Windows":
        ...
        if creds_file and os.path.exists(creds_file):
            ...
            print()
            print("To save permanently, open credentials.bat in Notepad and")
            print(f"set CANVAS_API_TOKEN={token}")
        return
