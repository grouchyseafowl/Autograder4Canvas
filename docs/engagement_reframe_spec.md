...
Starter diversity has the strongest theoretical stability claim because it stems from transformer architecture (repetition penalties, attention mechanisms) rather than training data or prompt style. If validated on a larger corpus (20+ AI essays across models), it would be a strong convergence channel. // SICK! great finding. The other pieces could even be cooberation signals that can be downplayed in population settings. 

**Validation path**: OpenRouter API (available in `GitHub/Reframe` project directory) can route prompts to multiple providers for efficient multi-model testing. Preference for free tier where available; also direct access to Gemini Pro and Claude Sonnet/Opus. Target: 20+ essays across 5+ models, in both discussion (200 word) and essay (500-1000 word) formats, with naive/roleplay/adversarial prompt variants. // oh you could even just use subagents wihtin claude code to provide anthropic generated responses. 

**Key finding**: Structural signals are **model-dependent for rhythm** (sentence uniformity) but potentially **model-stable for repetition avoidance** (starter diversity). ChatGPT tends rhythmic; Gemini Pro and Claude produce human-like rhythm but MAY still show the starter diversity and comma density patterns. Need more data to confirm. Structural signals are worth measuring as ONE input to engagement analysis, but they are NOT a reliable standalone detection mechanism. // yes i htink we should collect that additional data. 
...
**Note on contextual grounding**: The system currently uses GENERIC patterns ("In class when...", "The textbook on page...") — it does NOT pull actual course content from Canvas. This means it detects that someone *references a course context*, not that they reference the *right* content. Canvas API integration should target READING references (author names, work titles, specific concepts from assigned material), not just the assignment prompt — a student who feeds the prompt into ChatGPT will produce on-topic text, but WON'T reference the specific arguments/framings from the assigned readings. See Mechanism 2 (Assignment Context Awareness) in the Learning Through Use spec. // we just built a citation checker mechanic. See how that works - my vision was to 1) check whether citations are real sources (could also surface if they were from course readings or outside research potentially, but that's not built) and 2) surface individual and classwide patterns in the UX - eg what sources did the course engage with most, and how?
...
| **Class trajectory** | How the class's engagement evolves across assignments | To build | "The class is deepening/stagnating/diverging" | // this is really interesting to me. I wonder if we can make this qualitative as well as quantitative - *how* does the class engagement change over time? Could draw on qualitative analysis tools/libraries and/or llm (must work for 8b models, with potential to scale up to higher models if teachers have access)

**Equity framing for similarity**: Similarity can indicate strong community, collaborative learning, or shared cultural knowledge (#INTERDEPENDENCE, #COMMUNITY_CULTURAL_WEALTH) — not only copying. Surface as class-level pattern ("this assignment had unusually high similarity"), NOT as individual student flags. The first question is always: "Is the assignment designed to produce diverse responses?" // excellent. This is actually a great way to track ai use in my experience. When multiple students write similar things - especially when these diverge from how i taught the concept or when they all make the same mistake, this can be an AI signal (e.g., ai provides a general answer, whereas i was teaching them an answer from a partiuclar field. ) This signal COULD be AI, or it COULD be real collaboration. 
