...
BACKEND = BackendConfig(
    ...
    api_key=os.environ.get("REFRAME_SHARED_OPENROUTER_KEY", ""),
    api_format="openai",
)
...
# Gemma 3 on Google AI Studio doesn't support system prompts via OpenRouter.
# Fold system instruction into user prompt prefix instead.
_SYSTEM_TEXT = (
...
SYSTEM = ""  # empty — content folded into prompts
...
def send_with_retry(backend, prompt, system, max_tokens, max_retries=5):
    ...
    prompt = _SYSTEM_TEXT + prompt  # Gemma: fold system into user prompt
...
# Save full results
output_path = ROOT / "data" / "demo_baked" / "synthesis_first_prototype_27b.json"
